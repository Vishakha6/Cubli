...
#define GYRO_AVERAGE_OFFSET_X ((float) -52.22)
#define GYRO_AVERAGE_OFFSET_Y ((float) -72.83)
#define GYRO_AVERAGE_OFFSET_Z ((float) 8.59)
float gx_offset = -52.22;
float gy_offset = -72.83;
float gz_offset = 8.59;
/*
// Calibration example:
...
//PID controller on yaw//
 PID(Setpoint,TO_DEG(yaw));
 
 if (TO_DEG(yaw)>Setpoint)
 {
   Serial.println("forward");
   forward(motor_pwm);
   Serial.println(motor_pwm);
 }
 else if (TO_DEG(yaw)<Setpoint)
 {
